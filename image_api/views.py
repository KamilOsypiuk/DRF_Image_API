...
from image_api.serializers import ImageOutputSerializer, ExpirationLinkOutputSerializer, ImageSerializer, \
    ManyImagesOutputSerializer
...
class ImageApiView(GenericAPIView, RetrieveModelMixin):
    ...
    def get(self, request: Request) -> Response:
        ...
        try:
            images = service.return_image_sizes_based_on_tier(user_id=request.user.pk)
...
class ExpirationLinkApiView(GenericAPIView):
    ...
    def get(self, request: Request, link_id: UUID) -> Response:
        ...
        try:
            ...
            serializer = ImageSerializer(data=expiration_link.image)
            data = serializer.data
            return Response(data, status=status.HTTP_200_OK)
        except (ServiceException, ValidationError) as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
